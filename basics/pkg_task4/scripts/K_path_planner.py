    ...
    def obstacle_avoid(self):
        ...
        for i in range(len(data)-1):
            if(i==1):
                ...
                self.obs_closest_range=14.5
            if data[i] <= self.obs_closest_range:
                if i % 2 != 0:
                    self.movement_in_plane[0] = data[i] - self.obs_closest_range
                    
                    if(-0.0000107487 <= (self.given_destination.longitude-self.current_location[1])<= 0.0000107487) and (abs(self.lat_to_x(self.current_location[0]-self.given_destination.latitude))) > data[i]:
                        
                        self.movement_in_plane[1]=12
                    elif((abs(self.lat_to_x(self.current_location[0]-self.given_destination.latitude))) > data[i]):
                        print("oooohhhooooooooooooohhhhhhhhhhhhoooohhhhhhh")
                        self.movement_in_plane[1] = 6#self.movement_in_1D
                    break
            else:
        ...
        if(self.box_reach_flag and self.destination_switch==False):
            ...
            self.checkpoint.altitude = 24#self.destination_list[2][2]+3
        else:
            self.checkpoint.altitude = 24#self.destination_list[self.next_delevery][2]+3
